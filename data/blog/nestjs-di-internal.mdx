...
h1: 'Nestjs: Dependency Injection Internalsl'
date: '2022-11-25'
lastmod: '2022-11-25'
draft: false
...
사실 Nodejs에는 딱히 ‘구조’적인 서버사이드 프레임워크랄 게 없었다. 대표적으로 [Express](https://expressjs.com/ko/)는 자유도의 끝판왕이라 프로젝트가 커질수록 개발자 커뮤니티에서 암묵적인 패턴을 만드는 주객이 전도된(?) 상황이 발생하게 된다. Nestjs가 해결하고자 했던 문제가 바로 ‘구조’ 문제이고, 그러다보니 Spring과 유사한 모습을 띈다는 평가가 많다. 실제로 Nestjs는 under the hood에서 Express를 http server 프레임워크로 사용하고 이를 추상화해서 우리에게 제공해준다.
...
`bootstrap`은 또 app을 만들고, app을 listen하게 하는 간단한 함수다. 우리가 주목할 곳은 ‘app을 만드는’ 부분인 `NestFactory.create` 함수이다.

- [**@nestjs/core/nest-factory.ts#L65**](https://github.com/nestjs/nest/blob/master/packages/core/nest-factory.ts#L65)**: create**

`NestContainer` 는 모듈들의 모-든 정보를 담는 클래스다. `NestContainer` 의 인스턴스인 `container` 와 우리가 인자로 넘겨줬던 `AppModule` 인 `module` 을 가지고 `initialize` 함수를 호출한다.
...
![img](https://cdn-images-1.medium.com/max/1600/1*1mZ4ZSGyjPEzss6zrm6-Dw.png)Nodejs 디버거 콜스택

main.ts에서 bootstrap을 실행했고, `NestFactory.create` 함수 내부에서 `initialize` 를 실행했다. 이제 `initialize`에서 DependenciesScanner의 `scan` 메소드까지 실행해야 한다. `scan` 함수는 우리가 인자로 제공했던 `AppModule` 을 시작으로 모듈과 모듈의 의존성을 스캔한다.

[**@nestjs/core/scanner.ts#L70**](https://github.com/nestjs/nest/blob/220b098e220b1e3471493d036425d525a951e566/packages/core/scanner.ts#L70)**:**
...
`NestContainer` 는 모든 모듈의 모-든 정보를 담는 클래스다. `scanForModules` 는 `imports`를 이용해 모듈들을 순회하면서 container에 모듈의 정보를 담는 함수이다.
...
클래스에 메타데이터를 consistent한 방식으로 지정할 수 있도록 하는 [reflect-metdata](https://www.npmjs.com/package/reflect-metadata) 패키지가 있다. `Reflect.getMetadata`는 이 패키지에서 제공하는 API 중 하나다.

이 패키지의 내용을 표준에 추가하자는 수요가 많음에도, Decorator가 아직 TC39 프로세스 중 [Stage 3](https://github.com/tc39/proposal-decorators)에 있어서인지, ECMA 스크립트 표준에 이 패키지 적용을 제안하는 [proposal을 아직 제출하지 않은 것 같다.](https://github.com/rbuckton/reflect-metadata/issues/9)
...
즉 이 코드는 해당 모듈에 정의된 메타데이터 중 `imports`에 해당하는 값을 가져오는 코드다. 그럼 이 메타데이터는 어디서 정의하는 걸까? 여기서 Nestjs의 Decorator magic이 사용된다.
...
![img](https://cdn-images-1.medium.com/max/1600/1*8QwmQ5ANlOfrIf1ETIiFQw.png)AppModule 예시

`scanForModules`와 `scanModulesForDependencies`의 차이는 전자는 `NestContainer`에 module을 추가하는 과정이고, 후자는 그렇게 추가한 module에 controller, provider 등을 추가한다는 점이다.
...
container는 `NestApplication`의 프로퍼티로 접근 가능하고, `NestFactory.create`의 리턴값이 `NestApplication`의 프록시를 씌운 것이기 때문에 애플리케이션 코드에서 `app.container` 로 접근이 가능하다. 그래서 실제로 container에 접근해서 의존성 관계를 [시각적으로 보여주는 패키지](https://github.com/jmcdo29/nestjs-spelunker)도 있다.
...
*A progressive Node.js framework for building efficient, scalable, and enterprise-grade server-side applications on top…*github.com](https://github.com/nestjs/nest/)

[**진심으로 업무(?) 자동화 슬랙봇 만들기 (1)**
*우리 회사 사람들은 밥에 진심이다. 식대가 꽤나 높기도 하고 회사 근처 맛집이 많기 때문이기도 하다. 그리하여 슬랙에 "#bob" 채널을 만들어 메뉴를 의논하고 식사팀을 만들어 나가게 되었다. 문제는 매번 "오늘…*velog.io](https://velog.io/@hanchchch/진심으로-업무-자동화-슬랙봇-만들기-1)

[**GitHub - rbuckton/reflect-metadata: Prototype for a Metadata Reflection API for ECMAScript**
*Prototype for a Metadata Reflection API for ECMAScript - GitHub - rbuckton/reflect-metadata: Prototype for a Metadata…*github.com](https://github.com/rbuckton/reflect-metadata)
